...
export default function ReviewsPage() {
  ...
  return (
    <div>
      <h2 className="section-title" style={{ marginTop: 0 }}>Avis</h2>
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="row" style={{ gap: 10, alignItems: 'center' }}>
          <StarsDisplay value={data.avg} size={20} />
          <b style={{ fontSize: 18 }}>{data.avg.toFixed(1)}</b>
          <span className="small">({data.count} avis)</span>
        </div>
      </div>
      {data.reviews.length === 0 ? (
        <div className="empty">Pas encore d'avis sur tes livraisons.</div>
      ) : (
        data.reviews.map((r) => (
          <div className="card" key={r.orderId}>
            ...
            <div className="small" style={{ margin: '4px 0' }}>Commande chez {r.restaurantName}</div>
