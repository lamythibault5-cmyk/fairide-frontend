...
function Parametres({ tr, token, toast }) {
  ...
  useEffect(() => { const l = (lignes || []).find((x) => x.year === Number(annee)) || (lignes || [])[0]; if (l) setF({ p2pMaxGross: l.p2p_max_gross, studentMaxHours: l.student_max_hours, studentSolidarityRate: l.student_solidarity_rate, studentSolidarityEmployerRate: l.student_solidarity_employer_rate, studentOrdinaryRate: l.student_ordinary_rate, studentOrdinaryEmployerRate: l.student_ordinary_employer_rate, p2pWithholdingRate: l.p2p_withholding_rate, studentParentsCeiling: l.student_parents_ceiling, studentMinAge: l.student_min_age, franchiseMaxTurnover: l.franchise_max_turnover, adultMinAge: l.adult_min_age ?? 18, p2pForfaitRate: l.p2p_forfait_rate ?? 0.5, independentSocialRate: l.independent_social_rate ?? 0.205, independentComplementaryExemption: l.independent_complementary_exemption ?? 1922.16, independentMinQuarterly: l.independent_min_quarterly ?? 890.42, independentStarterQuarterly: l.independent_starter_quarterly ?? 459.82, studentIndependentExemption: l.student_independent_exemption ?? 8687, studentIndependentCeiling: l.student_independent_ceiling ?? 17374.08 }); }, [lignes, annee]);
  const champs = [['p2pMaxGross', '€'], ['studentMaxHours', 'h'], ['studentSolidarityRate', 'taux'], ['studentSolidarityEmployerRate', 'taux'], ['studentOrdinaryRate', 'taux'], ['studentOrdinaryEmployerRate', 'taux'], ['p2pWithholdingRate', 'taux'], ['studentParentsCeiling', '€'], ['studentMinAge', 'ans'], ['franchiseMaxTurnover', '€'], ['adultMinAge', 'ans'], ['p2pForfaitRate', 'taux'], ['independentSocialRate', 'taux'], ['independentComplementaryExemption', '€'], ['independentMinQuarterly', '€'], ['independentStarterQuarterly', '€'], ['studentIndependentExemption', '€'], ['studentIndependentCeiling', '€']];
